...
pub fn record_from_entry(entry: &CallLogEntry) -> CallLogRecord {
    CallLogRecord {
        id: entry
            .call_id
            .clone()
            .unwrap_or_else(|| format!("sha256:{}", sha256_hex(&record_key_bytes(entry)))),
        chat_id: entry
            .group_jid
            .clone()
...
mod tests {
    ...
    fn imports_and_lists_through_the_trait() {
        ...
        assert!(
            store
                ...
                .expect("lock")
                .iter()
                .all(|row| row.raw.is_some())
        );

        // Re-importing merges instead of duplicating.
        assert_eq!(
            import_call_log_into(&store, &entries).expect("re-import"),
            2
        );
    ...
    fn fills_direction_from_the_hosting_message_envelope() {
        ...
        assert!(
            call_log_entry_from_message_with_direction(&wa::Message::text("hi"), true).is_none()
        );
    }
}
